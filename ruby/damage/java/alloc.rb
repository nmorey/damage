            ...
            def write(output, libName, entry, pahole, params)
                ...
                entry.fields.each() { |field|
                  ...
                  when :sort
                    ...
                    output.printf("\t\t\tint max = 0;\n")
                    output.printf("\t\t\tfor (#{field.java_type} obj: _#{field.sort_field}) {\n")
                    output.printf("\t\t\t\tif (obj._#{field.sort_key} > max) max = obj._#{field.sort_key};\n")
                    output.printf("\t\t\t}\n")
                    output.printf("\t\t\t_#{field.sort_field}_by_#{field.sort_key} = new #{field.java_type}[max];\n")
                    output.printf("\t\t\tfor (#{field.java_type} obj: _#{field.sort_field}) {\n")
