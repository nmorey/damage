            ...
            def write(output, libName, entry, pahole, params)
                ...
                entry.fields.each() { |field|
                  ...
                  when :sort
                    ...
                    output.printf("\t\t_#{field.sort_field}_by_#{field.sort_key} = null;\n")
                    output.printf("\t\tif (_#{field.sort_field} != null && !_#{field.sort_field}.isEmpty()) {\n")
                    output.printf("\t\t\tint length = 0;\n")
                    output.printf("\t\t\tfor (#{field.java_type} obj: _#{field.sort_field}) {\n")
                    output.printf("\t\t\t\tif (obj._#{field.sort_key} >= length) length = obj._#{field.sort_key} + 1;\n")
                    output.printf("\t\t\t}\n")
                    output.printf("\t\t\tif (length > 0) {\n")
                    output.printf("\t\t\t\t_#{field.sort_field}_by_#{field.sort_key} = new #{field.java_type}[length];\n")
                    output.printf("\t\t\t\tfor (#{field.java_type} obj: _#{field.sort_field}) {\n")
                    output.printf("\t\t\t\t\tint key = obj._#{field.sort_key};\n")
                    output.printf("\t\t\t\t\tif (key >= 0)\n")
                    output.printf("\t\t\t\t\t\t_#{field.sort_field}_by_#{field.sort_key}[key] = obj;\n")
                    output.printf("\t\t\t\t}\n")
                    output.printf("\t\t\t}\n")
                    output.printf("\t\t}\n")
                    output.printf("\t}\n\n")
